...
const getScraperMap = function(visit: (o: ASTBase, level: number) => any): ScraperMap {
	return {
		'AssignmentStatement': function(item: ASTAssignmentStatement, level: number) {
		...
		'MapConstructorExpression': function(item: ASTMapConstructorExpression, level: number) {
			for (let fieldItem of item.fields) {
		...
		'ForGenericStatement': function(item: ASTForGenericStatement, level: number) {
			visit(item.variable, level);
		...
		'CallExpression': function(item: ASTCallExpression, level: number) {
            ...
            for (let argItem of item.arguments) {
				visit(argItem, level);
			}
		},
		...
		'ListConstructorExpression': function(item: ASTListConstructorExpression, level: number) {
			for (let fieldItem of item.fields) {
                visit(fieldItem, level);
            }
		},
		...
		'BinaryNegatedExpression': function(item: ASTUnaryExpression, level: number) {
			visit(item.argument, level);
		},
...
export function find(validate: ScraperValidate, rootItem: ASTBase): ASTBase | null {
    let result = null;
    const walker = new ScraperWalker((item: ASTBase, level: number) => {
...
export function findAll(validate: ScraperValidate, rootItem: ASTBase): ASTBase[] {
...
export function findAllByType(validate: ScraperValidate, rootItem: ASTBase): { [type: string]: ASTBase[] } {
